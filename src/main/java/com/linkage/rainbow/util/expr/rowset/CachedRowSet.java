...
import com.linkage.rainbow.util.expr.util.ClassInfo;
...
public class CachedRowSet extends BaseRowSet implements Serializable, Cloneable {
     ...
     * 
     */
    private static final long serialVersionUID = 8111200226448263014L;
	...
	public boolean absolute(int i) {
		if (i <= 0 || i > numRows) {
			throw new RuntimeException("absolute: Invalid cursor operation.");
		}
		cursorPos = i;
		return true;
	}
	...
	public void populate(java.util.List list, String strHeadCH, String strTag){
		if (StringUtils.hasText(strHeadCH)||StringUtils.hasText(strTag)) {
	...
	public BaseRow add(Object obj) {
		if (obj == null) {
			throw new RuntimeException("add(Object obj[]):追加出错:所要追加的记录不是有效.");
		}
...
//		row.setOrigRow(obj);
	...
	public void add(String strVal, String strTag) {
		if (strVal == null || strVal.trim().length() == 0 || strTag == null
				|| strTag.length() == 0) {
			throw new RuntimeException("add(String,String):Invalid columns Values");
		}
		/*
	...
	public void add(BaseRow baserow) {
		if (baserow == null) {
			throw new RuntimeException("add(BaseRow baserow):追加出错:所要追加的记录不是有效.");
		}
	...
	public void add(List list) {
		if (list == null || list.size() == 0) {
			throw new RuntimeException("add(List list):追加出错:所要追加的记录不是有效.");
		}
		add(list.toArray());
	}
	...
	private BigDecimal _getBigDecimal(Object i) {
		...
		try {
			...
			throw new RuntimeException("getDouble Failed on value ("
	...
	private boolean _getBoolean(Object i) {
		...
		try {
			...
			throw new RuntimeException("getBoolen Failed on value ("
					+ obj.toString().trim() + ") in column " + i);
		}
	}
	...
	private byte _getByte(Object i) {
		...
		try {
			...
			throw new RuntimeException("getByte Failed on value (" + obj.toString()
					+ ") in column " + i);
		}
	}
		...
		if (obj == null) {
			...
			case 0:
				try {
					...
					throw new RuntimeException("getDate Failed on value ("
							+ obj.toString().trim() + ") in column " + i);
				}
			case -1:
			case 1: // '\001'
	...
	private float _getFloat(Object i) {
		...
		try {
			...
			throw new RuntimeException("getfloat Failed on value ("
					+ obj.toString().trim() + ") in column " + i);
		}
	}
	...
	public int _getInt(Object i) {
		...
		try {
			...
			throw new RuntimeException("getInt Failed on value (" + obj.toString()
					+ ") in column " + i);
		}
	}
	...
	private long _getLong(Object i) {
		...
		try {
			...
			throw new RuntimeException("getLong Failed on value ("
					+ obj.toString().trim() + ") in column " + i);
		}
	}
	...
	private short _getShort(Object i) {
		...
		try {
			...
			throw new RuntimeException("getShort Failed on value ("
	...
	private Time _getTime(Object i) {
		...
		case 12: // '\f'
			try {
				...
				throw new RuntimeException("getTime Failed on value ("
						+ obj.toString().trim() + ") in column " + i);
			}
		}
		throw new RuntimeException("getTime Failed on value ("
	...
	public void setMetaHead(String[] strHead) {
		if (strHead == null || strHead.length == 0) {
			throw new RuntimeException("setMetaHead:Invalid columns name");
		}
	...
	public void setMetaHeadCH(String[] strHeadCH) {
		if (strHeadCH == null || strHeadCH.length == 0) {
			throw new RuntimeException("setMetaHeadCH:Invalid columns Comment");
		}
		 ...
		 */if (iLen != RowSetMDI.getColumnCount()) {
			throw new RuntimeException("setMetaHeadCH:字段注解数量与字段名数量不一致");
		}
		for (int i = 1; i <= iLen; i++) {
	...
	public void setHideHead(String strHeadHide) {
		if (!StringUtils.hasText(strHeadHide)) {
			return;
		}
	...
	private Object getColIdxByName(Object s) {
		if(s instanceof String){
			if(isArray){
				...
				throw new RuntimeException("找不到字段名为:" + s);
	...
	private Object convertTemporal(Object obj, int i, int j)
		...
		if (isNumeric(j) || !isString(j) && !isTemporal(j)) {
			throw new RuntimeException("Datatype Mismatch");
		}
		try {
			switch (j) {
			case 91: // '['
	...
	private void checkCursor() {
		if (isAfterLast() || isBeforeFirst()) {
			throw new RuntimeException("Invalid cursor position");
		} else {
			return;
		}
	}
	...
	public boolean first() {
		if (numRows <= 0) {
			throw new RuntimeException("First: Invalid cursor operation.");
		} else if (numRows > 0) {
			cursorPos = 1;
			return true;
		} else {
	...
	public boolean last() {
		if (numRows <= 0) {
			throw new RuntimeException("Last: Invalid cursor operation.");
		} else if (numRows > 0) {
			cursorPos = numRows;
			return true;
		} else {
