...
public abstract class BaseRowSet implements Serializable, Cloneable {
     ...
     * 
     */
    private static final long serialVersionUID = -6369533188119520161L;
	 ...
	 * Exception{ checkParamIndex(i); Object aobj[] = new Object[2]; aobj[0] =
			...
			if (iLen == 2) {
				if (objTemp[0] == null) {
					...
					if (objTemp[1] instanceof Calendar) {
						...
						throw new RuntimeException("Unable to deduce param type");
					}
				}
	...
	public void readData(CachedRowSet cachedrowset) {
		com.linkage.rainbow.util.expr.rowset.DataSource dataCon = null;
		...
		try {
			...
			userCon = false;
			if (dataCon == null) {
				dataCon = new com.linkage.rainbow.util.expr.rowset.DataSource(dsn);
			}
