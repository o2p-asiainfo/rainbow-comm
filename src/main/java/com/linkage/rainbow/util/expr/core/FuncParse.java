...
public class FuncParse {
    ...
    {
    	//数据集函数
    	//addDSFunction("select", "com.linkage.rainbow.util.expr.core.func.DBSelectFunc"); //数据集列表函数
    	//addDSFunction("group", "com.linkage.rainbow.util.expr.core.func.DBGroupFunc");//数据集分组函数
    	addDSFunction("selectone", "com.linkage.rainbow.util.expr.core.func.DBSelectOneFunc"); //数据集取一行函数
    	addDSFunction("select1", "com.linkage.rainbow.util.expr.core.func.DBSelectOneFunc"); //数据集取一行表函数
    	addDSFunction("sum", "com.linkage.rainbow.util.expr.core.func.DBSumFunc");//数据集统计函数
    	addDSFunction("count", "com.linkage.rainbow.util.expr.core.func.DBCountFunc");//数据集计数函数 
    	addDSFunction("avg", "com.linkage.rainbow.util.expr.core.func.DBAvgFunc");//数据集平均数函数
    	//addDSFunction("chart", "com.linkage.rainbow.util.expr.core.func.ChartFunc");//图表函数
    	//普通函数
    	addFunction("bias", "com.linkage.rainbow.util.expr.core.func.BiasFunc");//画表格斜线函数
    	addFunction("round", "com.linkage.rainbow.util.expr.core.func.RoundFunc");//四舍五入函数
    	addFunction("decode", "com.linkage.rainbow.util.expr.core.func.DecodeFunc");//decode判断函数
    	addFunction("case", "com.linkage.rainbow.util.expr.core.func.CaseFunc");//case判断函数
    	addFunction("to_number", "com.linkage.rainbow.util.expr.core.func.ToNumberFunc");//case判断函数
    	//addFunction("row", "com.linkage.rainbow.util.expr.core.func.RowFunc");//row函数,返回当前表格行数
    	//addFunction("col", "com.linkage.rainbow.util.expr.core.func.ColFunc");//col函数,返回当前表格列数
    	//addFunction("sum", "com.linkage.rainbow.util.expr.core.func.SumFunc");//画表格斜线函数
    	//addFunction("count", "com.linkage.rainbow.util.expr.core.func.CountFunc");//画表格斜线函数
    }
	...
	public static synchronized Func createInstance(String funName,String para,
			Report report) throws RuntimeException {
		...
		if (myFunc != null) {
			...
			throw new RuntimeException("无此函数:"+funName+".请检查语法是否正确!");
		}
	...
	public static synchronized Func createDSInstance(String dsName,String funName,String para,
		...
		if (myFunc != null) {
			...
			throw new RuntimeException("无此数据集函数:"+dsName+"."+funName+".请检查语法是否正确!");
		}
		return myFunc;
	}
...
//				len = para.length();
				...
				while (currIndex < len) {
					...
					case ']': // ')'
						if (--inBrackets < 0)
							throw new Exception("括号不匹配，右括号太多！");
						currIndex++;
						continue;
					...
					case '"': // '"'
						...
						if(currIndex == -1)
							throw new Exception("表达式异常,引号没有正常结束");
						currIndex++;
						break;
					case '\'': // '"'
						currIndex = Expr.findQuotationMark(para, currIndex); // 查询字符串结束
