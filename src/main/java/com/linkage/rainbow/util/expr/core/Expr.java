...
import com.linkage.rainbow.util.expr.core.expr.Add;
import com.linkage.rainbow.util.expr.core.expr.And;
import com.linkage.rainbow.util.expr.core.expr.BigThen;
import com.linkage.rainbow.util.expr.core.expr.Constant;
import com.linkage.rainbow.util.expr.core.expr.DBVariable;
import com.linkage.rainbow.util.expr.core.expr.Divide;
import com.linkage.rainbow.util.expr.core.expr.Equals;
import com.linkage.rainbow.util.expr.core.expr.GigEquals;
import com.linkage.rainbow.util.expr.core.expr.In;
import com.linkage.rainbow.util.expr.core.expr.LessEquals;
import com.linkage.rainbow.util.expr.core.expr.LessThen;
import com.linkage.rainbow.util.expr.core.expr.Mod;
import com.linkage.rainbow.util.expr.core.expr.Multiply;
import com.linkage.rainbow.util.expr.core.expr.Not;
import com.linkage.rainbow.util.expr.core.expr.NotEquals;
import com.linkage.rainbow.util.expr.core.expr.ObjVariable;
import com.linkage.rainbow.util.expr.core.expr.Or;
import com.linkage.rainbow.util.expr.core.expr.Subtract;
...
public class Expr implements Serializable{
     ...
     * 
     */
    private static final long serialVersionUID = -7521432478549250848L;
	...
	/**所有OBJ变量*/
	protected List objVarList = new ArrayList();
	/**所有常量*/
	 ...
	 * @param expStr  表达式字符串
	 * @param report 报表请求类
	 * @throws Exception 报表异常
	 */
	public Expr(String expStr,Report report){
		this.report = report;
		this.expStr = expStr;
	 ...
	 * @throws Exception 报表异常
	 */
	public Expr(String expStr,String dsName,CachedRowSet crs) {
	 ...
	 * @throws Exception 报表异常
	 */
	public Expr(String expStr,CachedRowSet crs) {
	 ...
	 * @throws Exception 报表异常
	 */
	public Expr(String expStr,Map para)  {
		this.expStr = expStr;
		this.para = para;
		create();
	}
	...
	protected Sign createSign() {
		...
		if (index > 0 && !Character.isDigit(id.charAt(0))) {		
			...
			if(sign != null)
                ...
                throw new RuntimeException("有不可识别的数据集函数" + colName);
		}
		if (id.charAt(0) == '$') {
			id = id.substring(1);
			ObjVariable objVar =null;
			if(para != null){
				objVar = new ObjVariable(para,id);
			} else {
				if(report != null)
					objVar = new ObjVariable(report.getPara(),id);
				else 
					objVar = new ObjVariable(id,null);
			}
			addObjVar(objVar);
			return objVar;
		}else if (id.charAt(0) == '@') {
        ...
        if(id.charAt(0) == '#'){
	        try {
	        	colIndex = Integer.parseInt(id.substring(1));
				 ...
				 throw new RuntimeException("数据集列索引格式不正确");
			}
...
//                	func.setParameter(scanParameter());
    ...
    protected String scanParameter()
    {
        ...
        if(currIndex == len)
            throw new RuntimeException("函数缺少参数");
        c = expStr.charAt(currIndex);
        if(c != '(')
            throw new RuntimeException("函数缺少参数");
	 ...
	 * @return 表达式结果
	 */
	public Object calculate(Map<String, Object> paraNew) {
		if (root == null)
			return null;

		return root.calculate(paraNew);
	}
	 ...
	 * @param constant 常量
	 */ 
	public void addObjVar(ObjVariable objVar) {
		objVarList.add(objVar);
	}
	...
	public static void main(String args[]) {
		...
		rp.setDs("list", rs);
		rp.setPara(para);
		//Expr e = new Expr( "\"area_id=\"+area_id+\"&stat=\"+stat+list.count(value)",new Report());
		//$value>
		Expr e1 = new Expr( "$value",rp);
		para.put("value", 100);
		Object o = e1.calculate();
		System.out.println(o);
		Expr e = new Expr( "case($value>list.avg(exa_value),'color:red','aa')",rp);
		for(int i=1;i<10;i++){
		...
		*/
		Map map = new HashMap();
		map.put("BUS_CODE", "BUS26100");
		map.put("SVC_CODE", "SVC26203");
		Expr e1 = new Expr( "$BUS_CODE != BUS26000");
	
		Object obj = e1.calculate(map);
		System.out.println(obj);
		e1 = new Expr( "$BUS_CODE=BUS26000 and $SVC_CODE=SVC26201");
		obj = e1.calculate(map);
		System.out.println(obj);
		e1 = new Expr( "$BUS_CODE=BUS26000 and $SVC_CODE != SVC26201");
		obj = e1.calculate(map);
		System.out.println(obj);
